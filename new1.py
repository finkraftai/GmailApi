...
def save_msg_attachment(attachment_data, filename, folder_path):
    ...
    try:
        # Create a Message object from the attachment data
...
def save_attachment(attachment_data, filename, folder_path):
    ...
    #     print(f"MSG attachment '{filename}' extracted and saved to '{folder_path}'.")
        ...
        # If the attachment is a MSG file, extract its contents using save_msg_attachment
...
def main():
    ...
    filters_label_id = get_label_id(service, user_id, "new1")
    if not filters_label_id:
        filters_label_id = create_label(service, user_id, "new1")
    
    # Count emails per day
    start_date = datetime(2024, 5, 17).strftime('%Y-%m-%d')
    end_date = datetime(2024, 5, 18).strftime('%Y-%m-%d')  # Note: We specify the day after the end date
    ...
    try:
        ...
        with open("emails.csv", "w", newline="", encoding="utf-8") as csvfile:
            fieldnames = ["From", "To", "Subject", "noofattachments", "noofattachments_downloaded"]
            ...
            for message in messages:
                ...
                writer.writerow({"From": from_email, "To": to_email, "Subject": subject, "noofattachments": attachment_count, "noofattachments_downloaded": downloaded_count})
